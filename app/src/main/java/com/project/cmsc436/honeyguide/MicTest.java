...
public class MicTest extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (soundPool != null) {
            soundID = soundPool.load(getApplicationContext(), R.raw.chirptest, 1);
        } else {
            Toast.makeText(getApplicationContext(), "Failed to load Chirp. Try again later.", Toast.LENGTH_LONG);
        }
        ...
        testButton = findViewById(R.id.testSpeakerButtonText);
        testButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

                if (ContextCompat.checkSelfPermission(getApplicationContext(), Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
                    ActivityCompat.requestPermissions(getParent(), new String[] {Manifest.permission.RECORD_AUDIO}, RESULT_REQUEST_RECORD_AUDIO);
                }
                else {
                    chirpConnect.start();
                }
